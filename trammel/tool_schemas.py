"""JSON Schema definitions for Trammel MCP tools.

Tool schemas live here (not in ``mcp_server``) so the handler/dispatch module
can stay under the project's 500-LOC target. ``mcp_server`` re-exports
``TOOL_SCHEMAS`` for backward compatibility with existing imports.
"""
...
TOOL_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "verify_step": _schema("verify_step",
        ...
        {"edits": _prop("array", "Edits for this step: [{path, content}, ...].",
         ...
         "language": _prop("string", "Project language for auto-detecting test command and error patterns.", enum=LANGUAGES)},
        ["edits", "project_root"]),
    ...
    "estimate": _schema("estimate",
        ...
        {"project_root": _prop("string", "Absolute path to the project root."),
         ...
         "language": _prop("string", "Override language detection.", enum=LANGUAGES)},
        ["project_root"]),
...
def coerce_int_params(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    """Coerce string values to int for schema-declared integer parameters.

    MCP clients may serialize numeric values as strings. We cast them here
    using the declared schema so handlers don't need per-field coercion.
    """
